...
class EmotionProcessor:
    ...
    def calculate_emotion_scores(self, llm_text: str) -> Dict[str, float]:
                ...
                if keyword in llm_text:
                    if word_type in ["强化词_高", "强化词_中"]:
                        ...
                        context_multiplier = min(
                            context_multiplier, self.scoring_weights[word_type]
                        )
                        print(
                            f"  发现弱化词 '{keyword}': 倍数调整为 {self.scoring_weights[word_type]}"
                        )
        ...
        for emotion, base_score in emotion_scores.items():
            if base_score > 0:
                ...
                if adjusted_score != base_score:
                    print(
                        f"  {emotion}: {base_score} → {adjusted_score} (×{context_multiplier} +{context_bonus})"
                    )
            else:
        ...
        if "default" in final_scores:
            ...
            if max_score < self.scoring_weights["最低阈值"]:
                ...
                print(
                    f"[情感计分] 所有得分都很低，default获得兜底分数: {self.scoring_weights['default兜底']}"
                )
    ...
    def check_send_probability(self, emotion_name: str, emotion_score: float) -> bool:
        ...
        if (
            ...
            or emotion_score < self.scoring_weights["最低阈值"]
        ):
        ...
        print(
            f"[概率控制] {prob_type}概率检查: {probability} vs {random_value:.3f} → {'发送' if will_send else '不发送'}"
        )
        return will_send
    ...
    def format_sse_response(self, emotion_folder: str, meme_filename: str) -> str:
        ...
        """
        # 从配置中获取URL前缀
        url_prefix = self.config.get("paths", {}).get(
            "expression_url_prefix", "/expression/memes/"
        )
